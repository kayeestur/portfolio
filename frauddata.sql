...
SELECT type, amount, nameorig, oldbalanceorg, newbalanceorig, namedest, oldbalancedest, newbalancedest, isfraud, isflaggedfraud,
CASE
	WHEN amount = oldbalanceorg AND newbalanceorig = 0 THEN 'Zeroed Out'
	WHEN amount > oldbalanceorg THEN 'Insufficient Old Balance'
	WHEN amount <> (oldbalanceorg - newbalanceorig) THEN 'Incorrect New Balance'
	ELSE 'For investigation'
END AS Fraud_Controls_Origin
INTO fraud_dataset
FROM fraud
WHERE isfraud=1

/* SUMMARY OF CONTROLS */
SELECT fraud_controls_origin, COUNT(fraud_controls_origin)
FROM fraud_dataset
GROUP BY fraud_controls_origin
ORDER BY COUNT(fraud_controls_origin) DESC
